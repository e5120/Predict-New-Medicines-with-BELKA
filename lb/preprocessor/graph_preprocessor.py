...
def from_smiles(smiles: str, with_hydrogen: bool = False,
    ...
    x = torch.tensor(xs, dtype=torch.uint8).view(-1, 9)
    ...
    edge_index = edge_index.t().to(torch.uint8).view(2, -1)
    edge_attr = torch.tensor(edge_attrs, dtype=torch.uint8).view(-1, 3)
